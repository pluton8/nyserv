...
;                            NewYear Service 1.2
;     ╧Ёюур-ёхЁтшё, ъюЄюЁр  яюърч√трхЄ ёъюы№ъю фэхщ/ўрёют/ўьё юёЄрыюё№ фю
;       эрёЄєяыхэш  ═ютюую ├юфр.
;     ┬ёх чрь█ўрэш , тюяЁюё√ ш яЁхфыюцхэш  яю яЁюух яЁшё√ырщЄх эр ьющ ь√ы:
...
.const
  crBkgnd equ 000000ffh         ; ЎтхЄ Їюэр юъэр
  ;crText equ 00000000h          ; ЎтхЄ ЇюэЄр
  TimerID equ 800
  TimerRefreshID equ 801
  milliseconds equ 5000         ; ўхЁхч ёъюы№ъю ьшыышёхъєэф юъэю чръЁюхЄё 
  reftime equ 500               ; тЁхь  юсэютыхэш 
...
.data
  ...
  szNY db "фю ═ютюую ├юфр", 13, 10, "юёЄрыюё№ ", 0
  szDays db "%s%lu фэхщ", 0
  szHours db "%s%lu ўрёют", 0
  ...
  szHelp db "NewYear Service 1.2", 0dh, 0ah
         db "╧юърч√трхЄ ёъюы№ъю юёЄрыюё№ фэхщ фю", 0dh, 0ah
         db "сышцрщ°хую ═ютюую ├юфр", 0dh, 0ah
         db "╧рЁрьхЄЁ√:", 0dh, 0ah
         db "/s - єёЄрэютшЄ№ ёхЁтшё", 0dh, 0ah
         db "/d - єфрышЄ№ ёхЁтшё", 0dh, 0ah
         db "/? - ¤ЄюЄ їхыя", 0
  crText COLORREF 0
...
GetNumParameters proc uses ebx edx
  xor eax, eax  ; ъюытю ярЁрьхЄЁют
  ;inc eax
  xor edx, edx  ; Їыру. хёЄ№ ыш ърт√ўъш
  mov ebx, cmdline  ; єърчрЄхы№ эр ярЁрьхЄЁ√
@b1:
  cmp byte ptr [ebx], 0
  je @endproc
  cmp byte ptr [ebx], '"'
...
; dwArgc - ъюышўхёЄтю рЁуєьхэЄют
; lpszArgv - єърчрЄхы№ эр ьрёёшт єърчрЄхыхщ эр ёЄЁюъш-рЁуєьхэЄ√
